...
export class DocumentStreamReader {
  ...
  constructor(document: TextDocument, pos?: Position, limit?: Range) {
    ...
    this._eof = limit
      ? limit.end
      : new Position(this.document.lineCount - 1, this._lineLength(this.document.lineCount - 1))
  ...
  peek(): number {
    ...
    return this.pos.character < line.length
      ? line.charCodeAt(this.pos.character)
      : this._eol.charCodeAt(this.pos.character - line.length)
  }
  ...
  backUp(n: number): number {
    ...
    this.pos = row < 0 || column < 0 ? new Position(0, 0) : new Position(row, column)
